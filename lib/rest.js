...
function fail(res,reason){ // 20121026 RR
    return res.send({
        ok:false,
        error:reason
    });
...
app.get('/:db/:collection/:id?', function(req, res){
    ...
    if(req.query.query) {
        try { // 20121026 RR
            ...
            return fail(res,'failed to parse query')
        }
    };

    // Providing an id overwrites giving a query in the URL
    if(req.params.id) {
        try { // 20121026 RR - support non-ObjectID keys
            query={
    ...
    for(o in req.query) {
        if(test.indexOf(o)>=0) {
            options[o]=JSON.parse(req.query[o]); // 20121026 RR
        }
    }

    var db=new mongo.Db(req.params.db,
        new mongo.Server(
            config.db.host,
            config.db.port,
            config.serveropts || {} // 20121026 RR
        ),
        config.dbopts || {} // 20121026 RR
    );
